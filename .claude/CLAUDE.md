...
## Explaining a GitHub issue or discussion (assessment format)

When asked to look at / check / verify / assess a GitHub **issue or
discussion** (as opposed to being told to implement it), the answer is an
*explanation*, not a code dump. Assume the reader has not read the reporter's
post and does not have the file layout in their head. Lead with the plain
meaning, not the architecture.

Answer in this order:

1. **What the reporter actually wants**, in one or two sentences of plain
   language. Strip the jargon from their post. Name them and link the prior
   issue/PR if it's a repeat of something already reported.
2. **A concrete walkthrough of the problem.** Pick one realistic setup and
   trace it step by step: "you open `tt.mydomain.com` → the dashboard loads →
   the page asks `tt.mydomain.com:8000` for data → nothing is listening →
   blank charts." Numbered steps beat a paragraph. The example must be
   specific (real hostnames, real ports, real endpoints), not "suppose a user
   does X".
3. **How it works today**, only the part that matters to the issue, with
   `file.py:line` anchors so it can be checked.
4. **What implementing it would take** — the shape of the change and which
   layer it belongs in, not a full diff.
5. **Traps and interactions**: security regressions, open PRs that touch the
   same files, closed issues with the same root cause, landing order.
6. **A verdict and one next step** ("worth accepting, want me to draft the
   reply comment?").

Rules for the prose:
- Explain the *symptom the user sees* before the mechanism ("it looks broken
  but nothing crashed — the page just never got its data").
- Any acronym or product name the reporter used (Pangolin, SSE, CORS) gets one
  clause of explanation the first time it appears.
- Don't paste large code blocks. Quote at most a few lines, or just cite
  `file:line`.
- If asked "what are the downsides", answer as **con → mitigation pairs**, and
  say plainly which cons are acceptable and which are blockers.
- State clearly whether you changed anything. An assessment ends with the tree
  untouched unless implementation was requested.
