...
async def get_analytics():
    from power_config import load_power_config, is_local_session, co2_for_session, default_tok_per_sec_for_model
    from billing_mode import get_mode
    pcfg = load_power_config()
    sessions = await get_sessions_cached(); by_agent = {}; by_day = {}; by_model = {}
    for s in sessions:
        agent = s["agent"]
        if agent not in by_agent: by_agent[agent] = {"input": 0, "output": 0, "cached": 0, "total": 0, "cost": 0.0, "co2_g": 0.0, "session_count": 0}
        ...
        is_local = is_local_session(
            ...
            billing_mode=get_mode(agent),
            config=pcfg,
        )
        co2 = 0.0
        if is_local:
            tps = s.get("tok_per_sec") or default_tok_per_sec_for_model(model_name)
            co2 = co2_for_session(st.get("output", 0), config=pcfg, tok_per_sec=tps)
        ...
        model_name_disp = model_name or f"{agent} (unknown)"
        if model_name_disp not in by_model:
            by_model[model_name_disp] = {"input": 0, "output": 0, "cached": 0, "total": 0, "cost": 0.0, "session_count": 0, "agent": agent}
        for k in ["input", "output", "cached", "total"]: by_model[model_name_disp][k] += st.get(k, 0)
        by_model[model_name_disp]["cost"] += scost
        by_model[model_name_disp]["session_count"] += 1
