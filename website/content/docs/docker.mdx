...
The container path relies on `${HOME}` being set, which it is not in plain `cmd.exe` or PowerShell. Run the `make` commands from inside WSL or Git Bash. Without it, the `~/.claude` mount resolves to an empty path and no sessions appear.
</Callout>
