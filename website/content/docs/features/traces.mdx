...
A Qoder trace header reads **CREDITS** where other agents read input/output/cached, plus **DELEGATED** when sub-agents did part of the work, and **TOKENS: not recorded**. Qoder writes a usage block with every token counter at zero and bills in credits, so the zeros are what Qoder reports rather than a failed scan. See [Billing modes](/docs/configuration/billing) for why they cannot be reconstructed.
