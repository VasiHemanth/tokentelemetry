...
Clicking a project card opens a five-tab detail view:
...
The Artifacts tab collects the deliverable artifacts agents produced across this project's sessions: pages Claude Code published as hosted artifacts (linked externally), and the task list / implementation plan / walkthrough documents Antigravity writes per session. Each entry shows the agent, title, description, and a link to the session that produced it. Artifacts published from git worktree sessions roll up to the main repository's card. See [Artifacts](/docs/features/artifacts) for the full picture.
