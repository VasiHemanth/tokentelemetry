...
1. **Wait for a session to complete.** TokenTelemetry reads log files after a session ends (or after it creates a new log entry). Active sessions may not appear until the next 15-second dashboard refresh.

2. **Confirm the log directory exists.** Each agent writes to a default location:
   - Claude Code: `~/.claude/projects/`
   - Codex: `~/.codex/sessions/`
   - Gemini CLI: `~/.gemini/`
   - Antigravity: `~/.gemini/antigravity/`, `~/.gemini/antigravity-ide/` and `~/.gemini/antigravity-cli/` (sessions in each store's `conversations/`)
   ...
   - Muse Code: `${XDG_DATA_HOME:-~/.local/share}/muse/sessions/`
   - Prime Agent: `~/.prime/agent/sessions/`
   - DeepSeek Harness: `~/.dsh/sessions/`
   - Qoder: `~/.qoder/projects/`
   - ZCode: `~/.zcode/cli/db/db.sqlite` (or `$ZCODE_DATA_DIR/cli/db/db.sqlite`)
   - Kimi Code: `~/.kimi/sessions/` (or `$KIMI_HOME/sessions/`)
...
3. **Run at least one session.** TokenTelemetry needs existing log files. Launch your agent, send a message, and wait for the session to complete before opening the dashboard.
...
5. **Restart TokenTelemetry.** If you installed the agent after starting TokenTelemetry, restart so the new log paths are picked up.

---

## Ports in use

**Symptom:** TokenTelemetry fails to start with "required port(s) already in use: 13000" (or 18000).
...
A single `-L 13000:...` forward without `-L 18000:...` produces a blank dashboard.
...
- [GitHub Discussions](https://github.com/VasiHemanth/tokentelemetry/discussions) — ask questions and share what you're seeing
- [GitHub Issues](https://github.com/VasiHemanth/tokentelemetry/issues) — report bugs with the dashboard or a specific agent
