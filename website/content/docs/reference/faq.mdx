...
- **SSH tunnel** with dual-port forward (`-L 13000:... -L 18000:...`) plus `NEXT_PUBLIC_API_BASE` on the remote side. The single-port command (`-L 13000:...` only) produces a blank dashboard.
...
Frontend (Next.js): 13000. Backend (FastAPI): 18000. Both are configurable — see [Ports & Networking](/docs/configuration/ports-networking).
...
Not yet. The backend exposes a REST API at `http://localhost:18000` that the frontend uses — it's undocumented and may change. A stable public API is tracked on the [issue tracker](https://github.com/VasiHemanth/tokentelemetry/issues).
