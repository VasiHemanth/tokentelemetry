...
![Summarizer backend picker showing Claude Code, Codex, Gemini CLI, Antigravity, Qwen Code, Ollama, OpenAI-compatible server, and Skip options](/screenshots/summarizer-backend.png)
...
The endpoint row carries one-click presets. Each fills in a base URL and a starting
model id so you only have to paste a key:

| Preset | Endpoint | Model |
| --- | --- | --- |
| **MiniMax** | `https://api.minimax.io/v1` | `MiniMax-M3` |
| **MiniMax (China)** | `https://api.minimaxi.com/v1` | `MiniMax-M3` |
| **OrcaRouter (referral)** | `https://api.orcarouter.ai/v1` | `orcarouter/fusion` |
| **Local (llama.cpp)** | `http://localhost:8080/v1` | _(whatever the server has loaded)_ |

A preset only fills the two fields. Any endpoint you type yourself keeps working
exactly as it did before.

### Trying several models for summaries

Summary quality varies a lot by model, and the only way to find out which one suits
your sessions is to run a few. Holding a separate key for every provider to do that
is tedious, so a gateway is the shorter path: one key, many models, one
OpenAI-compatible endpoint. [Explore models through OrcaRouter](/go/orcarouter) is
the preset shipped for this.

<Callout type="warn">
Two things to know before you click. That link is a referral, so TokenTelemetry
earns a share of what referred workspaces go on to spend; it costs you nothing, and
the preset works the same whether you use the link or sign up directly. Separately,
any hosted endpoint (a preset or one you typed) receives the prompts and responses
of every session you summarize. A local server does not.
</Callout>
