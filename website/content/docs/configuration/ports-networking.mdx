...
- **Frontend** (Next.js) — default port **13000**
- **Backend** (FastAPI) — default port **18000**

Both ports are configurable. The defaults sit away from 3000 and 8000, which most dev servers use, so TokenTelemetry can run next to your own projects. They match the Docker setup. Versions before September 2026 used 3000 and 8000; `./start.sh --port 3000 --api-port 8000` brings those back.
...
```bash
# Custom frontend port only (backend stays 18000):
./start.sh --port 4000
./start.sh -p 4000

# Custom backend port only (frontend stays 13000):
...
```
ERROR: required port(s) already in use: 13000
Stop whatever is listening on those ports and try again.
Tip: `lsof -iTCP:13000 -sTCP:LISTEN` shows the culprit.
```

On Windows:
```
Tip: `netstat -ano | findstr :13000` shows the culprit PID.
```
