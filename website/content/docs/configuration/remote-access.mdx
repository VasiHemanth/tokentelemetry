---
title: Remote Access
description: Expose the TokenTelemetry dashboard to other devices — tailnet, LAN, or SSH tunnel — with token-based authentication.
---
...
## SSH tunnel access (one forwarded port, VPS)
...
`--single-port` keeps FastAPI on its normal local port and streams API requests through the dashboard's same-origin proxy while preserving bearer headers and `?token=` artifact URLs.
...
This pattern requires no firewall changes and reuses your existing SSH authentication. No token is required because the tunnel reaches the frontend over loopback; real network callers are marked by the proxy and remain token-gated.
